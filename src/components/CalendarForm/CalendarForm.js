...
import {calculateDuration, formatTimeDisplay} from '../../utils/calendarUtils';
...
const CalendarForm = () => {
    ...
    const [timePickerOpen, setTimePickerOpen] = useState(null); // 'start' or 'end'
    ...
    const fetchAppointmentData = async () => {
        try {
            ...
            setFormData({
                ...appointment,
                // Truncate seconds from time strings (e.g., "09:00:00" -> "09:00")
    ...
    const handleDelete = async () => {
        ...
        try {
            setLoading(true);
            setError(null);
            await apiService.deleteCalendarAppointment(formData.calendar_id);
    ...
    return (
        <div className="calendar-form-container">
            <div className="header-container">
                <h1 className="modal-title">{isEdit ? 'Edit Appointment' : 'Add New Appointment'}</h1>
            </div>
                    ...
                    <div className="form-group">
                        ...
                        <div className="time-input-container">
                            <input
                                ...
                                name="start_time"
                                value={formatTimeDisplay(formData.start_time)}
                                onClick={() => setTimePickerOpen('start')}
                                readOnly
                                required
                            />
                            <span className="clock-icon" onClick={() => setTimePickerOpen('start')}>🕐</span>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="form-group">
                        ...
                        <div className="time-input-container">
                            <input
                                ...
                                className="time-input-field"
                                name="end_time"
                                value={formatTimeDisplay(formData.end_time)}
                                onClick={() => setTimePickerOpen('end')}
                                readOnly
                                required
                            />
                            <span className="clock-icon" onClick={() => setTimePickerOpen('end')}>🕐</span>
                        </div>
                    </div>
                </div>
            ...
            <TimePickerModal
                ...
                initialTime={timePickerOpen === 'start' ? formData.start_time : formData.end_time}
                inheritPeriodFrom={timePickerOpen === 'end' ? formData.start_time : null}
