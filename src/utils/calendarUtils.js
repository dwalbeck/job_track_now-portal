...
export const formatTimeDisplay = (timeString) => {
    ...
    return `${String(hour12).padStart(2, '0')}:${minutes} ${ampm.toLowerCase()}`;
};
...
export const getTimeSlots = () => {
    ...
    for (let hour = 0; hour < 24; hour++) {
        for (let minute = 0; minute < 60; minute += 15) {
            ...
            // Mark noon time (12:00 PM)
            const isNoon = hour === 12 && minute === 0;

            // Mark off-hours: 12am-8am (0-7) and 6pm-12am (18-23)
            const isOffHours = hour < 8 || hour >= 18;

            // Mark business hours: 8am-6pm (8-17), excluding noon
            const isBusinessHours = hour >= 8 && hour < 18 && !isNoon;
